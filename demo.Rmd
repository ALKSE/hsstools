...
params:
  ...
  grouping:
    ...
    value: gender
    input: select
    choices: [district, county, gender] 
  tables:
    label: "Table type"
    value: percent
    input: select
...
t
```
```{r table_multi, echo = FALSE, eval = FALSE}
t <- hsstable_multi(data, hssmultiresponses(question), params$grouping)
p <- hsschisq()
t <- flextable(t)
t <- hsslabel(t, question, dict_var, dict_val, lang = params$language, lookup = TRUE, lookuplist)
t <- hssformat(t)
...
```
<br><br>
```{r wage_type, echo = FALSE}
question <- "occupation_wagetype"
<<table_single>>
```
<br><br>
```{r income_hh, echo = FALSE}
question <- "incomehh2022"
<<table_single>>
```
<br><br>
