#' Write formatted tables to a list for selected variables (includes labels)
#'
...
#' @export
#'
T_hss_write_formatted_lab <- function(df, questions, group, dict, percent = TRUE, digits = 1, lang = "en") {
    ...
    tryCatch(
      if (names(questions[match(questions_element, questions)]) == "select_one") {
        T_hss_table_single(df, questions_element, group, percent, digits) %>%
          #T_hss_label may cause issues when it encounters NA values
